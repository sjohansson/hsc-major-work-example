...
The `get-nesa-grading-rules` skill in `.agents/skills/` loads the same two files. The facts were checked against NESA's pages on
...
- A path to a PDF: mark that file.
- Nothing: mark the current export, the PDF in `output/`.
...
This agent reads files, searches, runs commands, and writes only under `build/reviews/`. It needs a file-editing
tool for the review and its working files, and nothing else. It never edits anything under `design-system/`,
