...
tools: [read, search, execute, todo, web/fetch, vscode/askQuestions]
agents: []
---
...
- `tools:` is an allowlist. It names `read`, `search` and `execute` and leaves `edit` out, so this agent cannot
  change a file in the repository even if asked. `agents: []` also prevents this specialist from delegating.
  The missing edit tool and the refusal inside the scripts are the whole guard.
