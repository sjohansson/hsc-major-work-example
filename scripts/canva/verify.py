...
The export in `build/canva/canva-import-rev.html` is built by
`scripts/canva.py build`, which re-implements the deck.css cascade in
...
flattening changed shows up as ink where the reference has none.
    ...
    python scripts/canva.py verify
    python scripts/canva.py verify --scale 2 --keep build/canva/verify-2x
...
# label, in per cent). The one the folio ships with:
#
...
def export_pages(path: Path):
    ...
    for m in PAGE_RE.finditer(body):
        ...
        # The export lives beside design-system/, so its ./assets/ refs
        # already resolve; make them absolute for the temp file.
...
def shoot(chrome, html_path: Path, png_path: Path, scale: float):
    w = int(round(PAGE_W_MM / 25.4 * 96 * scale))
    h = int(round(PAGE_H_MM / 25.4 * 96 * scale))
    cmd = [
        chrome,
        "--headless=new",
        "--disable-gpu",
        ...
        f"--window-size={int(round(PAGE_W_MM / 25.4 * 96))},{int(round(PAGE_H_MM / 25.4 * 96))}",
...
def verify(export_path: Path = DEFAULT_OUT, chrome=None, scale: float = 1.0,
    ...
    for label, chunk in outs:
        ...
        ref_html.write_text(wrap(refs[label]), encoding="utf-8")
        out_html.write_text(wrap_export(chunk), encoding="utf-8")
        ...
        worst = max(worst, pct)
        limit = max(tolerance, KNOWN_RESIDUALS.get(label, 0.0))
        bbox = diff.getbbox()
        over = pct > limit
        if over:
            flag = "DIFF"
    ...
    if failed:
        ...
        return 1
    known = ", ".join(sorted(KNOWN_RESIDUALS)) or "none"
    print(f"  every page within {tolerance}%, except the known residual on {known}")
    return 0


def main(argv=None):
    ap = argparse.ArgumentParser(prog="canva.py verify", description=__doc__.splitlines()[0])
    ap.add_argument("--export", type=Path, default=DEFAULT_OUT)
    ap.add_argument("--scale", type=float, default=1.0)
    ap.add_argument("--tolerance", type=float, default=0.35)
    ap.add_argument("--chrome")
    ap.add_argument("--keep", type=Path, help="write the renders here instead of build/canva/verify")
